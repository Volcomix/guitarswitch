...
class Noise : public Articulation {
        enum Ports {
            NOISE_CHANNEL = 5,
            NOISE_NOTE    = 6
        };
        const float* noise_channel;
        const float* noise_note;

        bool noise_playing;

        void stop_noise(uint8_t velocity);
    protected:
    ...
    public:
        ...
        Noise(const LV2_Feature* const* features) : Articulation(features) { };
